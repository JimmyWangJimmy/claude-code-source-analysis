...
> 这一章专门从 Claude Code 的源码中提炼多 Agent 设计的深层启示。
> 不是"Claude Code 怎么做的"，而是"你该怎么做"。

Claude Code 可能是目前公开可见的**最成熟的生产级多 Agent 系统**。它不是学术论文里的 toy demo，而是每天被数十万开发者使用的真实产品。从它的源码中，我们可以提炼出一套完整的多 Agent 设计方法论。
...
这是 Claude Code 最精妙的设计之一——不是"完全隔离"或"完全共享"，而是**逐项控制**：
...
Claude Code 不是只有一种"spawn agent"——它有四种截然不同的模式，各有适用场景：
...
## 五、Fork Cache 共享：成本优化的天才设计

### Claude Code 怎么做

这是整个多 Agent 系统中最精妙的优化。
...
| 小数据 (<1KB) | ✅ 合适 | ❌ 过重 |
| 大数据 (>10KB) | ❌ 消息膨胀 | ✅ 合适 |
| 结构化数据 | ✅ JSON/类型 | ⚠️ 需约定格式 |
| 并发安全 | ✅ 消息队列天然有序 | ⚠️ 需文件锁 |
| 可追溯性 | ⚠️ 日志 | ✅ 文件版本 |
| 跨 Agent 共享 | ⚠️ 广播成本 | ✅ 天然共享 |
...
其中"边际收益检测"特别值得注意：当 Agent 连续轮次只产出少量 token（<500）时，系统判断 Agent 已进入无效循环并触发终止。
...
**启示 14：Agent 记忆按（Agent 类型 × 作用域）两个维度隔离。**
...
> **最后的话：** Claude Code 的多 Agent 设计给我最大的启示是——**多 Agent 系统的难点不在于"让 Agent 跑起来"，而在于"让 Agent 安全、高效、可控地协作"。** 这需要在隔离、通信、权限、成本、记忆每个维度上做精心的工程设计。源码中没有魔法——只有扎实的工程。
