# Claude Code 高效使用与隐藏玩法

> 从源码中挖出来的——官方文档里找不到的技巧、隐藏命令、高级配置和效率倍增器。
...
返回的不是对话文本，而是严格符合 schema 的 JSON。写脚本、做 CI 集成的利器。
...
让 Claude Code 同时看到多个目录的代码——跨仓库工作必备。
...
大多数人只用 Enter 和 Ctrl+C。但 Claude Code 有一套完整的快捷键系统：
...
每次都问你 "Allow Bash(git status)?" 烦不烦？在 settings.json 中配置一次，永不再问：
