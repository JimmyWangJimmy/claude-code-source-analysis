...
> **这是最有价值的部分之一。** Claude Code 的 system prompt 是 Anthropic 花费大量精力调优的产物。理解它的结构和策略，对构建任何 AI Agent 都有极大参考价值。
...
> **洞察:** 通过 prompt 解决 LLM "幻觉式编码"——强制模型在修改前建立完整上下文。
...
> **洞察:** Anthropic 观察到 LLM 有过度工程化的倾向 (加注释、加类型、提取函数)，用 prompt 显式抑制。
...
> **洞察:** 权限不是二元的。一次授权不等于永久授权——防止 Agent 越权的关键设计。
...
> **洞察:** LLM Agent 常见反模式——遇到错误就无脑重试。Prompt 明确禁止。
...
> **洞察:** 记忆可能过期，prompt 要求模型在使用记忆前先验证。
